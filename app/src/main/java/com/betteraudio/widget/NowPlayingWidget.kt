...
class NowPlayingWidget : BaseNowPlayingWidget() {
    ...
    override fun buildViews(
        ...
        // Idle + "hide when nothing is playing": keep just the background card (iv_bg above);
        // every control/text/cover element disappears and stops being tappable.
        val hideElements = hideWhenIdle && !state.isPlaying
        views.setElementsHidden(
            hideElements,
            R.id.tv_title, R.id.tv_author, R.id.iv_note,
