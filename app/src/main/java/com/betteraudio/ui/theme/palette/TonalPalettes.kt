package com.betteraudio.ui.theme.palette

/*
 * Ported from ArchiveTune (https://github.com/rukamori/ArchiveTune), GPL-3.0 License.
 * Original: © Rukamori — github.com/rukamori.
 */
...
/** Full tone ramp (0-100) per role, used to render the palette-picker preview. */
data class TonalPalettes(
    ...
    companion object {
        ...
        private fun Color.buildTonalMap(): Map<Int, Color> {
            val hct = toHct()
            return tonalTokens.associateWith { tone -> hct.withTone(tone.toDouble()).toColor() }
        }

        private fun Hct.buildReducedChromaMap(maxChroma: Double): Map<Int, Color> =
            tonalTokens.associateWith { tone -> Hct.from(hue, minOf(chroma, maxChroma), tone.toDouble()).toColor() }
    }
}
