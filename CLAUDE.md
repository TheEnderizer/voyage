...
A design (`data/db/entities/WidgetDesign.kt`, table `widget_designs`) is a `WidgetDesignDoc` (JSON, `widget/model/WidgetDesignDoc.kt`) — a background spec plus a z-ordered list of freely-positioned `ElementSpec`s in a fixed **design-unit space** (`CANVAS_UNITS = 1000` wide, `1000/aspectRatio` tall), so `w == h` is always a true square regardless of the design's own aspect ratio or the real granted pixel size. `WidgetPainter.paint(...)` is the single pure `(doc, aspectRatio, snapshot, sizePx) -> Bitmap` function shared verbatim by the provider, the editor's live canvas (`ui/widget/EditorCanvas.kt`), and every thumbnail (gallery, configure picker) — the editor preview IS the widget render, byte-for-byte. `WidgetBinding` (table `widget_bindings`) maps a placed `appWidgetId` to a design; `WidgetUpdater` handles orphan-binding GC, `onRestored` id remapping (launcher backup/restore), and re-rendering bound widgets as a "pick a design" placeholder when their design is deleted.

Tap handling can't use RemoteViews' per-view positioning below API 31 (minSdk 26), so `widget/HitGrid.kt` overlays a static 16×16 grid (`res/layout/widget_hit_grid.xml`, generated by `scripts/gen_hit_grid.py`) and assigns each interactive element's PendingIntent (`widget/WidgetIntents.kt`) to the cells its rendered rect actually overlaps (topmost z wins ties) — bounding tap error to about half a cell. TEXT/IMAGE/SHAPE elements can also carry a `TapAction` (play/pause, open player, skip, etc.), making a cover or title interactive too, not just dedicated control icons.

The editor (`ui/widget/WidgetEditorScreen.kt` + `WidgetEditorViewModel.kt`) is free-placement — no snapping; `ui/widget/AlignmentGuides.kt` draws purely visual center/thirds/edge hints during a drag that never move the element. `ui/widget/WidgetGalleryScreen.kt` is the design library (replaces the old inline Settings list); Settings → Widget now just links to it. New icons are the `ic_w_*` vector drawables (`widget/render/IconAssets.kt`) — full-bleed, tight glyphs with zero renderer-added padding, unlike the old `ic_*` set.
